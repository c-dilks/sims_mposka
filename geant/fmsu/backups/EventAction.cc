...
void EventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  for(G4int iv=0;iv<n_trajectories;iv++)
  {
    ...
    //BS. the primary particles are added in opssoiste order of what you would think. Also, added a storing cut on the decay mode in the PostUserTracking Action.
  ...
  for(UInt_t primary=0; primary <= dummy; primary++)
  {
    //Loops over all the primary particles, creating QT info for all the particles, then only those from each primary
    ...
    if(myCollectionL)
    {
      ...
      {
	if( primary!=0 && primary!= (*myCollectionL)[jj]->GetOriginalID() ) {continue;}
