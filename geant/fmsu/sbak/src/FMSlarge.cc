...
:G4VSensitiveDetector(name)
{
  ...
  if(calledFirst)
  {
    printf("<<<>>> FMSlarge::FMSlarge called\n");
    calledFirst = 0;
  };
  */
  G4String HCname;
...
void FMSlarge::Initialize(G4HCofThisEvent* HCE)
{
  ...
  */
  ...
  //FMSlargeHit * aHit = new FMSlargeHit();
...
{ 
  ...
  */
  ...
  /*
  //G4cout << "GETNAME = " << aStep->GetTrack()->GetVolume()->GetName() << G4endl;
  //G4cout << "OPTICAL = " << Optical << G4endl;
  G4TouchableHandle theTouchable = aStep->GetPreStepPoint()->GetTouchableHandle();
  G4int jjj = theTouchable->GetCopyNumber(2);
  //G4int jjj = theTouchable->GetCopyNumber();
  //G4int jjj = aStep->GetTrack()->GetVolume()->GetCopyNo();
  G4cout << "(" << cellgeo->GetNstb(jjj,1) << "," << cellgeo->GetRow(jjj,1) << "," << cellgeo->GetCol(jjj,1) << ")" << G4endl;
  G4cout << "   copyno=" << jjj << G4endl;
  G4cout << "trackdef=" << aStep->GetTrack()->GetDefinition() << " opticaldef=" << G4OpticalPhoton::OpticalPhotonDefinition() << G4endl;
  */
  ...
  else
  {
    ...
    {
      //G4cout<<"Unwanted Sensitive detector found ="<<aStep->GetTrack()->GetVolume()->GetName()<<G4endl;
      keepPhoto=true;
      isHit=false;
  ...
  /*
  G4int det = 1;
  if(firstphot && Optical)
  {
    printf("<><><> first optical photon detected <><><>\n");
    printf("motherCopyNo = %d\n",motherCopyNo);
    printf("nstb=%d row=%d col=%d\n",
      cellgeo->GetNstb(motherCopyNo,det),
      cellgeo->GetRow(motherCopyNo,det),
      cellgeo->GetCol(motherCopyNo,det));
    firstphot=0;
    initPhot=true;
  }
  */
  ...
  //G4cout << "                            hit inserted in large collection" << G4endl;
    //newHit->Print();
