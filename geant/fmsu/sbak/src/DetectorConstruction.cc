...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  myMPT8->DumpTable();
  Plexi->GetMaterialPropertiesTable()->DumpTable();
  ...
  // birks coefficients
  /*
  G4EmSaturation * emm = new G4EmSaturation();
  PbgL->GetIonisation()->SetBirksConstant(0);
  PbgS->GetIonisation()->SetBirksConstant(0);
  G4cout << "   birks coefficient PbgL = " << emm->FindG4BirksCoefficient(PbgL) << G4endl;
  G4cout << "   birks coefficient PbgS = " << emm->FindG4BirksCoefficient(PbgS) << G4endl;
  */

  // ---------------------------------------------- GEOMETRY
  // Import cookie length
  /*
  G4FileUtilities* fu = new G4FileUtilities();
  G4String css = fu->GetEnv("COOKIE");
  char ccc[10];
  sprintf(ccc,css);
  G4double cdd = atof(ccc);
  G4double CookieLength=cdd*mm;   // cookie length
  G4cout <<"Cookie Length = " << CookieLength << " mm" << G4endl; 
  */
  ...
  physiFMSL = new G4PVPlacement(0,
    ...
    false,
    0);
  G4cout << "FMS built" << G4endl;
  G4cout << "FMS LENGTH = " << (fFMSLengthL/2.+CookieLength+2*PhotoCLength)/cm << G4endl;
  ...
  physiCellS= new G4PVPlacement(0,
      ...
      false,
      0);
  G4cout << "Solid Cells built" << G4endl;
  ...
  physiPhotoCS= new G4PVPlacement(0,
      ...
      false,
      0);
  G4cout << "Photocathodes built" << G4endl;
  ...
  */
  ...
  G4cout<<"Cell param  begin "<<G4endl;
  ...
  G4cout<<"Cell param  stage1 "<<G4endl;
  ...
  // Mylar Surface
  G4cout<<"Cell param  stage2 "<<G4endl;
  G4int NUM=nEntries;
  ...
  // G4LogicalSkinSurface("mylar",logicCell,OpSurface);
  ...
  */
  ...
  /*
     G4MaterialPropertiesTable *OpSurfacePropertyC = 
     new G4MaterialPropertiesTable();
     for(int jj=0;jj<nEntries;jj++)reflectivity[jj]=0.;
     OpSurfacePropertyC -> AddProperty("REFLECTIVITY",ppp,reflectivity,NUM);
     OpSurfacePropertyC -> AddProperty("EFFICIENCY",ppp,efficiency,NUM);
     OpSurfaceC -> SetMaterialPropertiesTable(OpSurfacePropertyC);
  ...
  // Sensitive detectors
  //------------------------------------------------ 
  G4cout<<"Cell param  stage3 "<<G4endl;

  G4SDManager* SDman = G4SDManager::GetSDMpointer();
  //G4SDManager* SDmanS = G4SDManager::GetSDMpointer();
  G4String SDname;
  ...
  printf("[+] FMSlarge_sd SD @ %p\n",(void*)FMSlarge_sd);
  printf("[+] FMSsmall_sd SD @ %p\n",(void*)FMSsmall_sd);
  G4cout<<"Cell param  stage4 "<<G4endl;

  //--------- Visualization attributes -------------------------------

  G4VisAttributes* BoxVisAtt= new G4VisAttributes(G4Colour(1.0,1.0,1.0));
  G4cout<<"Cell param  stage4a "<<G4endl;

  logicWorld  ->SetVisAttributes(BoxVisAtt);  
  G4cout<<"Cell param  stage4b "<<G4endl;
  //  logicTarget ->SetVisAttributes(BoxVisAtt);
  G4cout<<"Cell param  stage5 "<<G4endl;
  logicFMSL->SetVisAttributes(BoxVisAtt);
  logicFMSS->SetVisAttributes(BoxVisAtt);
  G4cout<<"Cell param  stage6 "<<G4endl;
  ...
  //                                               minEkin));
  G4cout<<"return physiWorld "<<G4endl;
  return physiWorld;
}
...
void DetectorConstruction::setCellMaterial(G4String materialName)
{
  // search the material by its name 
  G4Material* pttoMaterial = G4Material::GetMaterial(materialName);  
  if (pttoMaterial)
  {CellMaterL = pttoMaterial;
    logicCellL->SetMaterial(pttoMaterial); 
    G4cout << "\n---------------------------------> The Cells are " << CellWidthL/cm << " cm of "
      << materialName << G4endl;
  }             
}
...
{
  if ((stepLimitL)&&(maxStep>0.)) stepLimitL->SetMaxAllowedStep(maxStep);
}
