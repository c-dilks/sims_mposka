...
const Float_t WWdefault = 1.44; // 2.31
...
void PlotSSmodel() {
  ...
  */
  ...
  for(Int_t t=0; t<nThetas; t++) {
    ...
    fn[t]->SetParLimits(kL, 0.5, 3.0);
    //fn[t]->SetParLimits(kW, 0.1, 4.0);
    fn[t]->FixParameter(kW, 4);
    ...
    fn[t]->SetParLimits(kVscale, 0.9, 1.1);
    //fn[t]->FixParameter(kHscale,1);
    //fn[t]->FixParameter(kVscale,1);
